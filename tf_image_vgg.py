...
if __name__ == '__main__':
  ...
  Y7 = tf.nn.relu(tf.nn.conv2d(Y6_drop, W7, strides=[1,1,1,1], padding='SAME')+b7)
  Y8 = tf.nn.max_pool(tf.nn.relu(tf.nn.conv2d(Y7, W8, strides=[1,1,1,1], padding='SAME')+b8), ksize=[1,2,2,1], strides=[1,2,2,1], padding='SAME')
  Y8_drop = tf.nn.dropout(Y8, keep_prob_1)


  YY = tf.reshape(Y8_drop, shape=[-1,2*2*NUM_FILTER_8])
